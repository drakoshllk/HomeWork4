...
public class HomeWork4 {
    ...
    public static List<String> correctedList(List<String> stringList) {
        ArrayList<String> correctedArray = new ArrayList<>();
        for (String arrayWord : stringList) {
            boolean wordIsRight = true;
            for (int j = 0; j < arrayWord.length(); j++) {
                if (Character.isUpperCase(arrayWord.charAt(j))) wordIsRight = false;
            }
            if (wordIsRight) correctedArray.add(arrayWord);
        }
        return correctedArray;
    }
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        HashMap<String, Integer> rightProducts = new HashMap<>();
        for (Map.Entry<String, Integer> product : shopCart.entrySet()) {
            boolean productIsRight = true;
            if (product.getValue() > 300) productIsRight = false;
            if (productIsRight) rightProducts.put(product.getKey(), product.getValue());
        }
